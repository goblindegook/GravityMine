...
class RGRedmineUpgrade{
    ...
    {
        $version_info = function_exists( 'get_site_transient' ) ? get_site_transient( 'gforms_redmine_version' ) : get_transient( 'gforms_redmine_version' );
    ...
    public static function display_upgrade_message($plugin_name, $plugin_title, $version, $message, $localization_namespace)
    {
        $upgrade_message = sprintf(
        	'%s <a class="thickbox" title="%s" href="plugin-install.php?tab=plugin-information&plugin=%s&TB_iframe=true&width=640&height=808">%s</a>. ',
        	$message, $plugin_title, $plugin_name, sprintf( __('View version %s Details', $localization_namespace), $version ) );
    ...
    public static function display_changelog ($offering, $key, $version) {
        ...
        $raw_response = wp_remote_request(LOGPT_UPGRADE_URL . "/changelog.php?" . self::get_remote_request_params($offering, $key, $version), $options);

        if ( is_wp_error( $raw_response ) || 200 != $raw_response['response']['code']){
            $page_text = sprintf( __("Something went wrong.%sPlease try again or %scontact us%s.", 'gravityformsredmine'), "<br/>", "<a href='http://log.pt'>", "</a>");
        }
        else{
            $page_text = $raw_response['body'];
            if(substr($page_text, 0, 10) != "<!--LOGUP-->")
                $page_text = "";
        }
    ...
    public static function get_version_info ($offering, $key, $version)
    {
        ...
        $url = LOGPT_UPGRADE_URL . "/version.php?" . self::get_remote_request_params($offering, $key, $version);
